...
## [2026-09-24] Review Follow-up: Remaining Dead Code, Wording & Bug Fixes
...
- `chore(release)` **v1.5.0**：**RCA** — (1) 版本號自 v1.4.0 後累積大量變更未升版；(2) `electron-builder.json` 的 `signAndEditExecutable: false`（專案初始化即存在）使 electron-builder 不編輯執行檔資源：內層 `ToDoCalendar.exe` 圖示為 Electron 預設、版本資訊顯示「Electron 33.4.11」，先前修正的圖示設定因而未生效。**CAPA** — `npm version 1.5.0`（UI 版本由 `__APP_VERSION__` 自動帶入）；啟用 `signAndEditExecutable`。**安裝檔驗證**：`npm run pack` 產出 `ToDoCalendar-Portable.exe`（75.5 MB，未簽章）；以 `--user-data-dir` 隔離設定檔（先確認可重導 `app.getPath('userData')`，不觸及使用者實際資料）並透過 DevTools 協定檢查內外兩個 exe：`file://` 載入、React 掛載、35 格日曆、側欄顯示 v1.5.0、`electronAPI` 可用且 `restoreWindow` 已移除、資料路徑位於隔離資料夾、不註冊 SW；兩個 exe 圖示皆為本專案圖示，版本資訊為 ToDoCalendar 1.5.0。**建置環境注意**：(a) 啟用資源編輯後 electron-builder 需解壓含 macOS 符號連結的 winCodeSign 工具包，Windows 須開啟開發人員模式或以系統管理員執行（README 已註明）；本次以 7za 排除 `darwin/` 手動解壓至建置快取處理。(b) 在 Claude 桌面應用（MSIX）中執行時 `%LOCALAPPDATA%` 寫入被虛擬化，electron-builder 快取搬移失敗，須以 `ELECTRON_BUILDER_CACHE` 指向一般資料夾；一般終端機不受影響。
- `fix(pwa)`（快取版本化）：**RCA** — Service Worker 快取名固定為 `todocalendar-v1`，各版本間從不變更，瀏覽器沒有理由安裝新 worker、舊快取也永不清除；且所有請求（含 `index.html` 導覽）一律 cache-first，更新後首次開啟必定顯示舊版。**CAPA** — `vite.config` 注入每次建置唯一的 `__BUILD_ID__`，註冊網址帶 `?v=<id>`，快取名隨建置變更、activate 時清除舊快取並 `clients.claim()`；頁面導覽改 network-first（離線才用快取），其餘資源維持 stale-while-revalidate；install 時解析 `index.html` 預先快取本次建置的 JS/CSS。**過程中發現並修正的自身缺陷**：初版未預快取，舊 worker 會把新 JS 寫入舊快取、隨即被新 worker 刪除，導致「更新後立即離線」白畫面。**驗證**（Electron 乾淨 Chromium、獨立伺服器，以兩份建置模擬部署）：首次載入即受控、部署後**一次重新整理即為新版**、部署後離線可正常開啟（35 格日曆）。註：內建瀏覽器的網路監看會繞過 SW，無法用於離線驗證；該 Electron 測試環境中 `caches.delete` 對存在的快取回傳 false，無法驗證清除舊快取，改於正式站部署後實測。
- `fix(pwa)`（預快取繞過 HTTP 快取）：**RCA** — 正式站實測發現新 worker 預快取的 `./index.html` 仍是上一版：GitHub Pages 回應 `Cache-Control: max-age=600`，install 時 `cache.addAll` 命中瀏覽器 HTTP 快取。`./` 因更新導覽時已刷新 HTTP 快取而正確，但 `./index.html`（離線備援鍵）會指向已不存在於新快取的舊 JS。**CAPA** — 預快取改用 `cache: 'reload'` 繞過 HTTP 快取，並直接由剛快取的 `index.html` 解析資源清單。以 `max-age=600` 的測試伺服器重現部署情境：兩個 HTML 鍵皆為新版、部署後離線正常。正式站實測：既有訪客由舊版 worker 升級時需 2 次重新整理（一次性），`todocalendar-v1` 與過渡快取皆已自動清除。
- `ci`（actions 升級）：**RCA** — 部署工作流程使用的 `checkout`/`setup-node`/`configure-pages`（v4）以 Node 20 執行，GitHub 已停用 Node 20 action runtime 並強制改以 Node 24 執行（每次部署出現棄用警告）；建置用的 Node 20 亦已於 2026-04 結束維護。**CAPA** — 升級至各 action 最新主版本（checkout v7、setup-node v7、configure-pages v6、upload-pages-artifact v5、deploy-pages v5，皆為 node24／composite），建置改用 Node 24 LTS。
- **還原 bundle 處置**：兩個含原始個資的還原 bundle 已刪除；刪除前先將歷史中全部 6 個資料匯出檔（含 207／208 筆任務的 6 月版本）逐一抽出並以 blob hash 驗證，保存於 repo 外的 `../ToDoCalendar-private-exports/`。已確認本機僅此一份 repo 副本（無其他 clone／worktree）；已備妥 GitHub Support 清除快取的申請內容（89 個舊提交含資料檔、0 個 PR）。
...
- `fix(types)`：清零全部 30 個既有 `tsc --noEmit` 錯誤（基準 45 → 上一輪後 30 → 0），型別檢查正式納入驗收閘門。內容：(1) 移除死引用——App 未使用的 `storageService` import 與 `handleReorderTasks`、Calendar 未使用的 `CategoryConfig` import 與 `onMonthChange` prop（月份切換按鈕在 App 側，Calendar 從未呼叫）、KanbanBoard 未使用的 `onReorder` prop、ReminderModal/TaskCard 多餘的 `React` 預設 import、useAppStore 未使用的 `get`；(2) 類型對齊——Calendar 的 `onDateSelect/onDateDoubleClick` 改為實際傳遞的 `Date`（App handler 本就收 `Date`）、TaskCard `onDelete` 收窄為 `(id: string)`（`Task.id` 恆為 string，消除 3 處上下游不匹配）、`catch (err)` 的 unknown 收窄、NotificationOptions 補 `vibrate` 斷言；(3) 移除 `(React as any)` 反模式（Calendar ×2、TaskListView ×4），恢復完整型別推導，連帶消除 5 處 implicit any；(4) i18n `getTranslation` 維持字串回傳契約，陣列值鍵（weekdays）於消費端斷言。驗證：`tsc --noEmit` 0 錯誤、85 測試全過、`vite build` 成功。
- `build(lint)`：**RCA** — `eslint.config.js`（flat config）引用未安裝的 `eslint-plugin-react`／`@eslint/js` 套件，且 lint script 的 `--ext` 旗標與 flat config 不相容，ESLint 完全無法執行（上一輪 Act 項）。**CAPA** — 改用與 ESLint 8.57 相容的 legacy `.eslintrc.cjs`（`@typescript-eslint` v7 parser/plugin + react-hooks + react-refresh，皆為既有 devDependencies，零新增依賴）；務實關閉 `no-undef`（由 tsc 把關）、`no-explicit-any`、`exhaustive-deps` 等既有風格規則並附註理由；`--fix` 自動修正 4 處 `prefer-const`。`npm run lint` 現以 0 錯誤 0 警告通過，恢復為可用的品質閘門。
- `docs`：同步 README 與 CONSOLIDATED_DOCUMENTATION（docs/ 為文件 SSOT，wiki repo 不存在）——i18n 鍵數 230+ → 270+（實際 274）、技術棧新增 Testing/Lint 列、架構圖標註單元測試、新增「Quality Gates」章節（四道閘門與通過門檻、deploy.yml 對應）、Recent Updates Log 新增本輪稽核條目（死碼清單、三個 bug 修復、閘門恢復歷程、backup 取消追蹤之隱私說明）。
